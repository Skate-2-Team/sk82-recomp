...
inline T *CreateKernelObject(Args &&...args)
{
    ...
    return g_heap->AllocPhysical<T>(std::forward<Args>(args)...);
}
...
inline T *GetKernelObject(uint32_t handle)
{
    ...
    return reinterpret_cast<T *>(Memory::Translate(handle));
}
...
{
    return reinterpret_cast<T *>(Memory::Translate(GUEST_INVALID_HANDLE_VALUE));
}
...
inline T *QueryKernelObject(XDISPATCHER_HEADER &header)
{
    ...
    if (header.WaitListHead.Flink != OBJECT_SIGNATURE)
    {
        ...
        header.WaitListHead.Blink = Memory::MapVirtual(obj);

        return obj;
    }

    return static_cast<T *>(Memory::Translate(header.WaitListHead.Blink.get()));
}
