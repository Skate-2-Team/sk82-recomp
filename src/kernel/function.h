...
/*
This whole file is from Unleashed Recompiled, full credit to the original developers.
https://github.com/hedge-dev/UnleashedRecomp/blob/main/UnleashedRecomp/kernel/function.h
*/
...
struct ArgTranslator
{
    ...
    constexpr static std::enable_if_t<!std::is_pointer_v<T>, void> SetValue(PPCContext &ctx, uint8_t *base, size_t idx, T value) noexcept
    {
        ...
        {
            SetIntegerArgumentValue(ctx, base, idx, Memory::MapVirtual(value));
        }
        else
        {
    ...
    constexpr static std::enable_if_t<std::is_pointer_v<T>, void> SetValue(PPCContext &ctx, uint8_t *base, size_t idx, T value) noexcept
    {
        const auto v = Memory::MapVirtual((void *)value);
        if (!v)
        {
            return;
        }
...
T GuestToHostFunction(const TFunction &func, TArgs &&...argv)
{
    ...
    _translate_args_to_guest(newCtx, Memory::g_base, args);

    SetPPCContext(newCtx);

    if constexpr (std::is_function_v<TFunction>)
        func(newCtx, Memory::g_base);
    else
        Memory::FindFunction(func)(newCtx, Memory::g_base);
    ...
    {
        return reinterpret_cast<T>((uint64_t)Memory::Translate(newCtx.r3.u32));
    }
